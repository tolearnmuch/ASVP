...
class VideoPredictionModel(BaseVideoPredictionModel):
    ...
    #
    def tower_fn(self, inputs, inputs_major, inputs_inferior):
        """
        ...
        if self.discriminator_fn:
            ...
            with tf.variable_scope(discrim_scope, reuse=True):
                discrim_outputs_post = self.discriminator_fn(inputs, inputs_major, inputs_inferior, gen_outputs)   # x
        else:
            discrim_outputs = {}
        ...
        if self.mode == 'train':
            ...
            with tf.name_scope("generator_loss"):
                g_losses = self.generator_loss_fn(inputs, inputs_major, inputs_inferior, outputs)
                ...
                if discrim_outputs_post:
                    ...
                    g_losses_post = self.generator_loss_fn(inputs, inputs_major, inputs_inferior, outputs_post)
                else:
    ...
    def build_graph(self, inputs, inputs_major, inputs_inferior):
        # self.inputs = inputs
        ...
        if self.num_gpus <= 1:  # cpu or 1 gpu
            ...
            outputs_tuple, losses_tuple, loss_tuple, metrics_tuple = self.tower_fn(self.inputs, self.inputs_major, self.inputs_inferior)
    ...
    def generator_loss_fn(self, inputs, inputs_major, inputs_inferior, outputs):

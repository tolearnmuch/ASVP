...
def main():
    ...
    parser.add_argument("--save_freq", type=int, default=1000, help="save frequence of model, 0 to disable")
    ...
    inputs_ap = iterator_major.get_next()
    # additional code 2: inferior
    ...
    inputs_nap = iterator_inferior.get_next()
	#inputs_ap
    # visualize the inputs, inputs_ap and inputs_nap first.
    vis1 = False
    if vis1:
        sess = tf.Session()
        with sess.as_default():
            ...
            for len_i in range(len):
                for bs_i in range(bs):
                    # take images from tensors. inputs, inputs_ap and inputs_nap
                    vis_inputi, vis_inputmi, vis_inputii = inputs['images'][len_i, bs_i, :, :, :].eval(), \
                                                           inputs_ap['images'][len_i, bs_i, :, :, :].eval(), \
                                                           inputs_nap['images'][len_i, bs_i, :, :, :].eval()
                    # visualize inputs
                    # mm = np.max(vis_inputi)
                    vis_inputi = np.uint8(vis_inputi * 255 / np.max(vis_inputi))
                    plt.imshow(vis_inputi)
                    # visualize inputs_ap
                    vis_inputmi = np.uint8(vis_inputmi * 255 / np.max(vis_inputmi))
                    plt.imshow(vis_inputmi)
                    # visualize inputs_nap
    ...
    model.build_graph(inputs, inputs_ap, inputs_nap)
    ...
    if long_val_dataset is not None:
        ...
        long_model.build_graph(long_val_dataset.make_batch(batch_size), long_val_dataset.make_batch(batch_size), long_val_dataset.make_batch(batch_size))
    else:
        long_model = None
